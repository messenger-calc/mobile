...
export default function Messanger() {
  ...
  const [currentWindow, setCurrentWindow] = useState<"Chat" | "Profile">(
    "Profile"
  );
  ...
  const [autoDeleteMode, setAutoDeleteMode] = useState<TAutoDeleteMode | null>(
    null
  );
  ...
  useEffect(() => {
    issueDetector.current?.eventEmitter.on(
      EventType.NetworkScoresUpdated,
      (res) => {
        setPacketsLoss(res.statsSamples.inboundStatsSample?.packetsLoss);
        socket.emit("packetLoss", {
          to: callFrom,
          networkType: "networkScore",
          remoteLoss: res.statsSamples.inboundStatsSample?.packetsLoss,
        });
      }
    );
  ...
  useEffect(() => {
    if (showCallingScreen) {
      axios
        .get("https://call-back-production.up.railway.app/sendPushNotification")
  ...
  const startCall = (isCaller: boolean, remoteId: string, config: any) => {
    setTimeout(() => {
      if (!connection) {
        setCalling(false);
      }
    ...
    const _pc = new PeerConnection(remoteId, security.current!)
      ...
      .start(isCaller, { audio: true, video: false });

    _pc.listenMessages(onMessageReceive);
    setPc(_pc);
    setCallFrom(remoteId);
    _pc?.pc?.addEventListener("iceconnectionstatechange", () => {
    ...
    axios
      .post(
        ...
        },
        {
          headers: {
            Authorization:
              "key=AAAAPzCbeHE:APA91bGT7SHI7HQZbQGK29znrqk9xNfEwaftALwqOLVY4oL-J2KAk_REy-rVkXYioBDXMV_KyTLFDEZv9t3NrSH55jbo2gsxk8R9FECU6j3IrbzEO49AEwC0J3U_Ce3xKT3M_Bwn0eg5",
          },
        }
      )
  ...
  return (
    ...
    //     {calling && <Calling startBeep={startBeep} />}
    //     {showModal && (
    //       <CallModal
    //         callFrom={callFrom}
    //         startCall={startCall}
